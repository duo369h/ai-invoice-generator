...
-- DRAFT R9 replacement for 20260826010904 on the evolved Production schema.
-- Production already has price_id and paddle_customer_id but lacks the
-- paddle_price_id column referenced by the historical R2 function body.
-- Add that genuinely missing compatibility column before installing the
-- otherwise unchanged R2 ordering/idempotency contract.
...
-- Preserve semantic uniqueness already supplied by a constraint-backed index.
-- Do not key this decision only on an index name: a valid full unique index
-- on exactly billing_events(event_id) is sufficient and must not be doubled.
DO $$
DECLARE
  v_event_id_attnum SMALLINT;
  v_has_valid_unique BOOLEAN;
  v_named_index_exists BOOLEAN;
BEGIN
  SELECT a.attnum INTO v_event_id_attnum
  FROM pg_attribute a
  WHERE a.attrelid = 'public.billing_events'::regclass
  ...
  SELECT EXISTS (
    SELECT 1
    FROM pg_index ix
    WHERE ix.indrelid = 'public.billing_events'::regclass
      AND ix.indisunique
      AND ix.indpred IS NULL
      AND ix.indexprs IS NULL
      AND ix.indnkeyatts = 1
      AND ix.indkey[0] = v_event_id_attnum
  ) INTO v_has_valid_unique;

  IF NOT v_has_valid_unique THEN
    SELECT EXISTS (
      SELECT 1
      FROM pg_class i
      JOIN pg_namespace n ON n.oid = i.relnamespace
      WHERE n.nspname = 'public'
        AND i.relname = 'billing_events_event_id_unique'
        AND i.relkind = 'i'
    ) INTO v_named_index_exists;

    IF v_named_index_exists THEN
      RAISE EXCEPTION
        'billing_events_event_id_unique exists but is not a valid full unique event_id protection';
    END IF;

    CREATE UNIQUE INDEX billing_events_event_id_unique
