      ...
      from: (table) => {
        ...
        return {
          update: (payload) => {
            ...
            return {
              eq: (col1, val1) => {
                if (col1 === "id") updateFilterId = val1;
                if (col1 === "user_id") updateFilterUserId = val1;
                return {
                  eq: (col2, val2) => {
                    if (col2 === "id") updateFilterId = val2;
                    if (col2 === "user_id") updateFilterUserId = val2;
                    return {
                      select: () => ({
                        single: async () => ({ data: { id: "inv_target_01", status: "sent" }, error: null })
                      })
                    };
                  }
                };
