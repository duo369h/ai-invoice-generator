...
const screenshotDir = path.join(cwd, 'output/playwright/r56e-g-qe-device-01');
...
async function assertMode(page, width, expected) {
  ...
  assert.equal(await page.getByTestId('quote-business-editor').isVisible(), true, `${width}: editor bridge remains reachable`);
...
console.log(`R56E-G-QE-DEVICE-01 browser runtime: PASS (${screenshotDir})`);
