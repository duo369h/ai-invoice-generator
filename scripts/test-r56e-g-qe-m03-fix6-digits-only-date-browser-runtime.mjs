...
const outputDir = path.join(cwd, 'output/playwright/r56e-g-qe-m03-fix6');
...
async function typeDigitsSequentially(page, locator, digits, expectedValues) { const observed = []; await clearField(locator); for (const character of digits) { assert.notEqual(character, '-', 'digits-only test never sends a hyphen'); await page.keyboard.type(character); await page.waitForTimeout(25); const reflected = await locator.inputValue(); observed.push(reflected); assert.equal(reflected, expectedValues[observed.length - 1], `digits-only prefix ${digits.slice(0, observed.length)} formats deterministically`); } return observed; }
...
try {
  ...
  try {
    ...
    const portraitObserved = await typeDigitsSequentially(page, shootDate, '20261010', ['2', '20', '202', '2026', '2026-1', '2026-10', '2026-10-1', '2026-10-10']);
    await page.screenshot({ path: path.join(outputDir, '390-portrait-date-digit-complete.png'), fullPage: true });
    await clearField(shootDate);
    await page.keyboard.type('202610');
    assert.equal(await shootDate.inputValue(), '2026-10', 'partial digit draft remains visible');
    await page.screenshot({ path: path.join(outputDir, '390-portrait-date-digit-partial.png'), fullPage: true });
    await clearField(shootDate);
    await page.keyboard.type('20261010');
    assert.equal(await shootDate.inputValue(), '2026-10-10', 'complete digit date remains visible');
    await leaveDate(shootDate);
    evidence.push({ workflow: 'Portrait', field: 'shoot_date', digitsOnly: '20261010', hyphensSent: 0, observed: portraitObserved, partialDigits: '202610', canonicalAfterCommit: '2026-10-10' });
    await shootDate.fill('20261010');
    assert.equal(await shootDate.inputValue(), '2026-10-10', 'unformatted digit paste formats');
    await leaveDate(shootDate);
    await shootDate.fill('2026-10-10');
    assert.equal(await shootDate.inputValue(), '2026-10-10', 'formatted paste remains stable');
    await leaveDate(shootDate);
    await shootDate.click();
    await shootDate.press('End');
    await shootDate.press('Backspace');
    assert.equal(await shootDate.inputValue(), '2026-10-1', 'backspace removes a digit without trapping at separator');
    await shootDate.press('Backspace');
    assert.equal(await shootDate.inputValue(), '2026-10', 'backspace continues across separator');
    await shootDate.click();
    await shootDate.press('End');
    await page.keyboard.type('10');
    assert.equal(await shootDate.inputValue(), '2026-10-10', 'backspace correction restores the formatted date');
    await leaveDate(shootDate);
  ...
  for (const workflow of ['Wedding', 'Commercial']) {
    ...
    try {
      ...
      const observed = await typeDigitsSequentially(page, locator, '20261010', ['2', '20', '202', '2026', '2026-1', '2026-10', '2026-10-1', '2026-10-10']);
      await leaveDate(locator);
      assert.equal(await locator.inputValue(), '2026-10-10', `${workflow} canonical date commit`);
      evidence.push({ workflow, field: 'shoot_date', digitsOnly: '20261010', hyphensSent: 0, observed, canonicalAfterCommit: '2026-10-10' });
      if (workflow === 'Commercial') {
        ...
        const deadlineObserved = await typeDigitsSequentially(page, deadline, '20261020', ['2', '20', '202', '2026', '2026-1', '2026-10', '2026-10-2', '2026-10-20']);
        await leaveDate(deadline);
        assert.equal(await deadline.inputValue(), '2026-10-20', 'Commercial delivery deadline canonical commit');
        evidence.push({ workflow, field: 'delivery_deadline', digitsOnly: '20261020', hyphensSent: 0, observed: deadlineObserved, canonicalAfterCommit: '2026-10-20' });
      }
  ...
  try {
    ...
    await goToScope(page, server.baseUrl, 'Portrait');
    const locator = page.getByTestId('quote-guided-scope-shoot_date');
    await typeDigitsSequentially(page, locator, '20260230', ['2', '20', '202', '2026', '2026-0', '2026-02', '2026-02-3', '2026-02-30']);
    await leaveDate(locator);
    ...
    await typeDigitsSequentially(page, clearable, '20261010', ['2', '20', '202', '2026', '2026-1', '2026-10', '2026-10-1', '2026-10-10']);
...
fs.writeFileSync(path.join(outputDir, 'digits-only-date-evidence.json'), JSON.stringify({ inputMode: 'numeric', sequentialInputString: '20261010', hyphenCharactersSentByTest: 0, visibleFinal: '2026-10-10', canonicalFinal: '2026-10-10', digitsOnlyPartialFormatting: 'PASS', validDateCanonicalCommit: 'PASS', invalidDateCanonicalRejection: 'PASS', dateBackspaceUsability: 'PASS', dateClearUsability: 'PASS', formattedPaste: 'PASS', unformattedDigitPaste: 'PASS', evidence }, null, 2));
console.log(`R56E-G-QE-M03-FIX-6 digits-only date browser runtime: PASS (${outputDir})`);
