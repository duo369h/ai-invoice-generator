...
export default function ForPhotographersV1() {
  ...
  useEffect(() => {
    ...
    if (!finePointer.matches) return () => { closeMobile(); cleanups.forEach((cleanup) => cleanup?.()); };

    const heroSec = root.querySelector('#sec-hero'); const heroUiCol = root.querySelector('#heroUiCol'); const quoteArtifact = root.querySelector('#quoteArtifact');
    cleanups.push(listen(quoteArtifact, 'pointerenter', () => { heroSec?.classList.add('is-hero-scene-active'); heroUiCol?.classList.add('is-scene-active'); }));
    cleanups.push(listen(quoteArtifact, 'pointerleave', () => { heroSec?.classList.remove('is-hero-scene-active'); heroUiCol?.classList.remove('is-scene-active'); }));
    ...
    const reviewSurface = root.querySelector('#clientReviewSurface'); const reviewContent = root.querySelector('.review-content-area'); const statusPanel = root.querySelector('#approvalStatusPanel'); const awaiting = root.querySelector('#statusAwaiting'); const accepted = root.querySelector('#statusAccepted'); const acceptedWrap = root.querySelector('#statusAcceptedWrapper'); const staticRail = root.querySelector('#statusStateRail'); const railFill = root.querySelector('#statusRailFill'); const statusGutter = root.querySelector('#statusGutter'); const statusTextColumn = root.querySelector('#statusTextColumn'); const reviewSections = { REVIEW_SCOPE: root.querySelector('#reviewScope'), REVIEW_DELIVERABLES: root.querySelector('#reviewDeliverables'), REVIEW_USAGE: root.querySelector('#reviewUsage'), REVIEW_TOTAL: root.querySelector('#reviewTotal') };
    const updateStatusGutterGeometry = () => { const awaitingName = root.querySelector('#statusAwaiting .status-state-name'); const acceptedName = root.querySelector('.status-accepted-name'); const dotAwaiting = root.querySelector('#statusDotAwaiting'); const dotAccepted = root.querySelector('#statusDotAccepted'); if (!awaitingName || !acceptedName || !statusGutter || !dotAwaiting || !dotAccepted || !staticRail || !railFill) return; const gutterRect = statusGutter.getBoundingClientRect(); const awaitingRect = awaitingName.getBoundingClientRect(); const acceptedRect = acceptedName.getBoundingClientRect(); const awaitingCenterY = awaitingRect.top + (awaitingRect.height / 2) - gutterRect.top; const acceptedCenterY = acceptedRect.top + (acceptedRect.height / 2) - gutterRect.top; dotAwaiting.style.top = `${(awaitingCenterY - 3).toFixed(1)}px`; dotAccepted.style.top = `${(acceptedCenterY - 3).toFixed(1)}px`; const fullRailHeight = Math.max(1, acceptedCenterY - awaitingCenterY); const seedRatio = Math.min(0.5, 14 / fullRailHeight); staticRail.style.top = `${awaitingCenterY.toFixed(1)}px`; staticRail.style.height = `${fullRailHeight.toFixed(1)}px`; statusPanel?.style.setProperty('--seed-scale-y', seedRatio.toFixed(3)); };
    if (reviewSurface) { if (document.fonts?.ready) document.fonts.ready.then(updateStatusGutterGeometry); else window.setTimeout(updateStatusGutterGeometry, 200); cleanups.push(listen(window, 'resize', updateStatusGutterGeometry)); }
    if (reviewSurface && reviewContent && statusPanel) { let currentM04State = 'REST'; let lastReviewState = null; const getStatusCorridorBounds = () => { if (!statusGutter || !statusTextColumn) return null; const gutterRect = statusGutter.getBoundingClientRect(); const textRect = statusTextColumn.getBoundingClientRect(); return { corridorLeft: gutterRect.left - 4, corridorRight: textRect.right + 10 }; }; const isWithinRect = (x, y, rect, paddingX = 8, paddingY = 5) => x >= rect.left - paddingX && x <= rect.right + paddingX && y >= rect.top - paddingY && y <= rect.bottom + paddingY; const resolveM04State = (x, y) => { const surfaceRect = reviewSurface.getBoundingClientRect(); if (x < surfaceRect.left || x > surfaceRect.right || y < surfaceRect.top || y > surfaceRect.bottom) return 'REST'; const corridor = getStatusCorridorBounds(); if (corridor && x >= corridor.corridorLeft && x <= corridor.corridorRight) { const awaitingRect = awaiting?.getBoundingClientRect(); const acceptedRect = accepted?.getBoundingClientRect(); if (awaitingRect && acceptedRect && y >= awaitingRect.top - 6 && y <= acceptedRect.bottom + 6) { lastReviewState = null; return isWithinRect(x, y, acceptedRect) ? 'STATUS_ACCEPTED_PREVIEW' : 'STATUS_AWAITING_FOCUS'; } } const contentRect = reviewContent.getBoundingClientRect(); if (x >= contentRect.left && x <= contentRect.right && y >= contentRect.top && y <= contentRect.bottom) { for (const [state, section] of Object.entries(reviewSections)) { const rect = section?.getBoundingClientRect(); if (rect && x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom) { lastReviewState = state; return state; } } return lastReviewState || 'REVIEW_SCOPE'; } return 'REST'; }; const applyM04 = (state) => { if (currentM04State === state) return; currentM04State = state; const reviewing = state.startsWith('REVIEW_'); reviewContent.classList.toggle('has-active-review', reviewing); Object.entries(reviewSections).forEach(([key, section]) => section?.classList.toggle('is-active-review', key === state)); statusPanel.classList.toggle('is-reviewing-awaiting', reviewing); statusPanel.classList.toggle('is-awaiting-focus', state === 'STATUS_AWAITING_FOCUS'); statusPanel.classList.toggle('is-previewing-accepted', state === 'STATUS_ACCEPTED_PREVIEW'); acceptedWrap?.classList.toggle('is-accepted-preview', state === 'STATUS_ACCEPTED_PREVIEW'); }; cleanups.push(listen(reviewSurface, 'pointermove', (event) => applyM04(resolveM04State(event.clientX, event.clientY)))); cleanups.push(listen(reviewSurface, 'pointerleave', () => { lastReviewState = null; applyM04('REST'); })); }

    const continuity = root.querySelector('#projectContinuitySurface'); const backbone = root.querySelector('#contextBackbone'); const quote = root.querySelector('#continuityQuoteRegister'); const invoice = root.querySelector('#continuityInvoiceRegister'); const contextRow = root.querySelector('#continuityContextRow'); const contextText = root.querySelector('#continuityContextText');
    if (continuity && backbone && quote && invoice) { let currentM05State = 'REST'; const applyM05 = (state) => { if (currentM05State === state) return; currentM05State = state; continuity.classList.remove('is-context-focus', 'is-quote-focus', 'is-invoice-focus', 'is-cross-reference'); if (state === 'CONTEXT_FOCUS') continuity.classList.add('is-context-focus'); else if (state === 'QUOTE_FOCUS') continuity.classList.add('is-quote-focus'); else if (state === 'INVOICE_FOCUS') continuity.classList.add('is-invoice-focus'); else if (state === 'INVOICE_CROSS_REFERENCE') continuity.classList.add('is-cross-reference'); }; const resolveM05State = (event) => { const target = event.target; if (contextRow && (contextRow.contains(target) || target === contextText)) return 'INVOICE_CROSS_REFERENCE'; if (backbone.contains(target)) return 'CONTEXT_FOCUS'; if (quote.contains(target)) return 'QUOTE_FOCUS'; if (invoice.contains(target)) return 'INVOICE_FOCUS'; const pointerX = event.clientX; if (pointerX < backbone.getBoundingClientRect().right) return 'CONTEXT_FOCUS'; if (pointerX < quote.getBoundingClientRect().right) return 'QUOTE_FOCUS'; return 'INVOICE_FOCUS'; }; cleanups.push(listen(continuity, 'pointermove', (event) => applyM05(resolveM05State(event)))); cleanups.push(listen(continuity, 'pointerleave', () => applyM05('REST'))); }
