    ...
    return (
      <>
        <style>{`
          ...
          @media (max-width: 820px) { .public-v2-navbar { padding-left: 20px; padding-right: 20px; } .public-v2-nav, .public-v2-signin, .public-v2-cta { display: none; } .public-v2-mobile-trigger { display: inline-flex; } .public-v2-mobile-menu { position: absolute; top: 64px; left: 0; right: 0; display: flex; flex-direction: column; gap: 4px; padding: 12px 20px 20px; border-bottom: 1px solid var(--border-medium, #e2e8f0); background: #f8fafc; box-shadow: 0 12px 20px rgba(15,23,42,.08); } .public-v2-mobile-menu[hidden] { display: none; } .public-v2-mobile-menu a, .public-v2-mobile-menu button { min-height: 44px; display: flex; align-items: center; padding: 10px 12px; border: 0; border-radius: 6px; background: transparent; color: var(--text-muted, #64748b); font: inherit; font-size: .95rem; font-weight: 650; text-align: left; text-decoration: none; } .public-v2-mobile-menu .public-v2-signin { display: flex; } .public-v2-mobile-menu > div { display: flex; flex-direction: column; gap: 4px; } .public-v2-mobile-menu a:hover, .public-v2-mobile-menu a:focus-visible, .public-v2-mobile-menu button:hover, .public-v2-mobile-menu button:focus-visible { background: var(--brand-light, #eef2ff); color: #4f46e5; outline: none; } .public-v2-mobile-menu .public-v2-mobile-cta { justify-content: center; margin-top: 6px; background: #4f46e5; color: #fff; } .public-v2-mobile-menu .public-v2-mobile-cta:hover, .public-v2-mobile-menu .public-v2-mobile-cta:focus-visible { background: #4338ca; color: #fff; } }
          ...
          <div className="public-v2-nav">
            {navLinks.map((link) => {
              ...
              <div className="public-v2-nav-group" key={`${link.href}-${link.label}`}>
                <button type="button" id={`public-v2-${link.label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-trigger-${idSeed}`} className="public-v2-nav-trigger" aria-expanded={publicDropdownOpen === link.label} aria-controls={`public-v2-${link.label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-panel-${idSeed}`} onClick={() => setPublicDropdownOpen((open) => open === link.label ? null : link.label)}>
        ...
        <nav id={mobileMenuId} className="public-v2-mobile-menu" aria-label="Mobile navigation" aria-labelledby={mobileTriggerId} hidden={!menuOpen}>
          {navLinks.map((link) => {
            ...
            if (!children.length) return <Link href={link.href} key={`mobile-${link.href}-${link.label}`} onClick={closeMenu}>{link.label}</Link>;
            ...
            return <React.Fragment key={`mobile-${link.href}-${link.label}`}>
              <button type="button" aria-expanded={open} aria-controls={panelId} onClick={() => (isHow ? setMobileHowOpen : setMobileResourcesOpen)((current) => !current)}>{link.label}</button>
              <div id={panelId} role="region" hidden={!open} aria-label={`${link.label} links`}>
                {children.map((child) => <Link href={child.href} key={`mobile-${child.href}-${child.label}`} onClick={closeMenu}>{child.label}</Link>)}
              </div>
            </React.Fragment>;
          })}
          {accountAction && renderPublicAction(accountAction, 'account')}
