/**
 * Pricing Controller — Corvioz v5.7
 *
 * Handles Side Effects only: checkout setups, navigation, and analytics events.
 * Keeps presentation component pure and free of external business couplings.
 */
...
export async function handleUpgradeCheckout(context: CheckoutContext): Promise<void> {
  ...
  const targetPlanObj = plans.find((p) => p.id === planId);
  const priceId = targetPlanObj
    ? (billingPeriod === 'monthly' ? targetPlanObj.paddle_monthly_price_id : targetPlanObj.paddle_yearly_price_id)
    : '';
