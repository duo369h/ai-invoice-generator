...
--   version : 20260711192505
--   name    : first_revenue_loop_payment_completion
...
--   dc889be9b93a6106893b172e592044e08f8d0157cd514536055f1e1bf7d1bf8a
--
-- Candidate source (4 byte-identical independent copies):
--   1. dirty workspace  supabase/migration-first-revenue-loop-payment-completion.sql
--   2. Desktop Corvioz-SAFE-03B0-20260725-045031/source-snapshot/supabase/
--   3. Desktop Corvioz-SAFE-03B2H-A-20260725-151250/candidates/C2__...
...
--   version 20260711192505, computed server-side, is byte-for-byte identical
--   to the SHA-256 of this file's body.
--
-- Superseded by later migrations: NONE (this is the newest of the three).
--   This migration defines the function versions that production runs TODAY.
--   Verified byte-for-byte against pg_proc.prosrc:
--     record_invoice_payment             body SHA-256 deee8910...e6d75c27  MATCH
--     claim_first_revenue_invoice_draft  body SHA-256 582ef31b...c1875239  MATCH
--   It replaced the versions created by 20260711181554.
--
-- Why this must never be re-run:
--   The WITH payment_totals ... UPDATE public.first_revenue_loops backfill
--   recomputes `stage` UNCONDITIONALLY from the current invoices.payment_status
--   for every loop joined to an invoice. first_payment_received_at and
--   completed_at are COALESCE-protected, but `stage` is not: a loop currently
--   at 'complete' whose invoice is no longer payment_status='paid' would be
--   silently DEMOTED to 'invoice_created'. Re-running rewrites derived
--   first-revenue lifecycle state from whatever the ledger happens to say at
--   that moment, which is not the same as replaying history.
--
--   Ordering hazard: this file must never be re-applied BEFORE 20260711181554
--   in any replay scenario, and 20260711181554 must never be re-applied AFTER
--   this one — doing so regresses both functions defined here.
--
